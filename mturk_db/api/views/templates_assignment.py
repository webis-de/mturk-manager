...
class Templates_Assignment(APIView):
    ...
    def get(self, request, slug_project, database_object_project, use_sandbox, format=None):
        # import time
        # time.sleep(2)
        queryset_settings_batch = Manager_Templates_Assignment.get_all_for_project(database_object_project.id)
...
class Template_Assignment(APIView):
    ...
    def get_object(self, id_template):
        try:
            return Model_Template_Assignment.objects.get(id=id_template)
        except Model_Template_Assignment.DoesNotExist:
            raise Http404
    ...
    def put(self, request, slug_project, database_object_project, use_sandbox, id_template, format=None):
        ...
        serializer = Serializer_Template_Assignment(project, data=request.data, partial=True)
        if serializer.is_valid():
            serializer.save()
            return Response(serializer.data)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
