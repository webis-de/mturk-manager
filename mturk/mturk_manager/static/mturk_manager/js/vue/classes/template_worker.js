...
export default class Template_Worker {
  constructor(data = {}) {
    ...
    this.name = data.name;
    this.height_frame =
      data.height_frame != undefined ? data.height_frame : 800;
    ...
    this.dict_parameters =
      data.json_dict_parameters != undefined
      ...
      // if(template_worker[key] != undefined)
      {
        if (this[key] != template_worker[key]) {
          if (typeof template_worker[key] == "object") {
            if (
              _.differenceBy(template_worker[key], this[key], value =>
                value["text"].toLowerCase()
              ).length > 0 ||
              _.differenceBy(this[key], template_worker[key], value =>
                value["text"].toLowerCase()
              ).length > 0
            ) {
