    ...
    <v-layout row>
        <v-flex>
            ...
            <v-text-field
                ...
                v-on:input="count_assignments_max_per_worker = $event; $v.count_assignments_max_per_worker.$touch()"
                label="Number of Maximal Assignments Per Worker"
                min="1"
                clearable
                v-bind:error-messages="validation_errors.count_assignments_max_per_worker"
            ></v-text-field>
        </v-flex>
        <v-flex shrink>
            <v-btn 
...
export default {
    ...
    methods: {
        ...
        ...mapActions('moduleProjects', {
            'set_count_assignments_max_per_worker': 'set_count_assignments_max_per_worker',
        }),
    },
    computed: {
        changed() {
