...
export default {
    ...
    computed: {
        title_button: function() {
            ...
            {
                return `Download all (${this.qualifications_without_database_entry.length}) Qualification(s)`;
            } else {
                return `Download ${this.qualifications_selected_without_database_entry.length} Qualification(s)`;
            }
        },
        qualifications_selected_without_database_entry: function() {
            return _.filter(this.qualifications_selected, q => q.has_database_entry());
        },
    ...
    methods: {
        download: function() {
            ...
            if(this.qualifications_selected_without_database_entry.length == 0)
            {
                for (let i = 0; i < this.qualifications_without_database_entry.length; i++) {
                    array_json.push(this.qualifications_without_database_entry[i].get_json());
                }
            } else {
                for (let i = 0; i < this.qualifications_selected_without_database_entry.length; i++) {
                    array_json.push(this.qualifications_selected_without_database_entry[i].get_json());
                }
            }
