...
export default class View
{
	...
	update_info()
	{
		...
		const sum = _.sum([count_approved_assignments, count_rejected_assignments, count_rejected_internally_assignments, count_approved_internally_assignments]);
	...
	inject_templates()
	{
		...
		let object_assignments_grouped_by_batch = _.groupBy(this.loader.object_assignments, 'hit.batch.id');
		_.forEach(object_assignments_grouped_by_batch, (list_assignments, id_batch) =>
		{
			...
			_.forEach(object_groups, (group) =>
			{
				...
				_.forEach(object_grouped_by_hits, (grouped_by_hits, id_hit) =>
				{
					...
					const wrapper_hit = $($.parseHTML('<span data-id_hit="'+id+'"></span>'));
					...
					_.forEach(grouped_by_hits, (assignment) =>
					{
						...
						const script_define_variables = $(`
							<script>
								var assignment_wrapper = $(\'${is_table ? 'tr': 'span'}[data-id_assignment="${assignment.id}"]\'); 
						...
						if(is_table == true)
						{
							...
							wrapper_assignment = $(`<span data-id_assignment="${assignment.id}"></span>`);
						...
						wrapper_assignment.find('[data-inject_input_forms]').append(this.create_inputs(assignment));
						// console.log(wrapper_assignment.find('data-inject_input_forms'));
	...
	create_inputs(assignment)
	{
		...
		_.forEach(this.loader.array_messages_reject, (message_reject) => {
			messages_reject += `<a class="dropdown-item" data-id_assignment="${assignment.id}" href="#">${message_reject.message}</a>`;
		});
